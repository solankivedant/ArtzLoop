---
name: new-toy
description: Scaffold a new ArtzLoop generative art toy from the master build prompt in artzloop.md. Use when starting to build one of the 20 toy ideas, or any new toy added to the list later.
---

# New toy

Scaffolds a single toy from the "Master build prompt" template in [artzloop.md](../../../artzloop.md).

## Steps

1. Ask (or infer from the user's request) which toy idea from the numbered list in `artzloop.md` to build, if not already specified.
...
3. Fill in the master build prompt template's bracketed placeholders:
   - `APP_NAME` - a short, specific name for the toy.
   ...
   - `EXT` and `[default-name]` - use the shared `.artz` extension unless the user has said this toy is a standalone sale item (see `artzloop.md`'s "one extension or many" note), in which case pick a toy-specific extension.
...
6. Before reporting the toy done, follow the testing steps in `.claude/rules/workflows.md` - open it in a browser and actually exercise draw → save → reload → load.
