...
Applies to every tool in `apps/` and to `shared/` and `gallery/`, unless a tool's own spec explicitly overrides it.

## Stack

- Each tool lives in its own `apps/<NN>-<kebab-case-name>/` folder as three files: `index.html`, `style.css`, `script.js` - linked with plain `<link rel="stylesheet">` / `<script src="script.js">` tags (no `type="module"`), so the folder still works from `file://` or any static host with no build step and no server.
  - A Vite + vanilla TypeScript project is acceptable if a tool needs component structure, as long as the shipped artifact is still a static bundle with no backend.
...
- Container: a renamed `.zip` (shared `.art` extension across all tools unless a tool is being sold standalone, in which case it may use its own extension - see `artzloop.md` notes).
...
- No sign-in, accounts, or telemetry/analytics in any tool.
