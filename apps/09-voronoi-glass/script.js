...
/* #19 Voronoi Stained Glass - click to add glass panes; cells regrow around
 * every point, with dark lead lines between them. Drag moves a pane. */
(function(){
let api, sites = [], needsRender = true, dragIdx = -1;
let small, sm, RES = 3;
const PALETTES = {
  jewel: ["#1f6feb","#b23b8f","#d29922","#2ea043","#a371f7","#db6d28","#39c5cf"],
  warm:  ["#9a2f2f","#c05c2e","#d8973c","#e3b448","#7a3b2e","#b56548"],
  cool:  ["#1d3557","#2a6f97","#468faf","#61a5c2","#89c2d9","#5e60ce"],
  forest:["#2d6a4f","#40916c","#52b788","#74c69d","#95d5b2","#606c38"],
};
function addSite(x, y){
  const pal = PALETTES[api.P.pal];
  sites.push({ x:Math.round(x), y:Math.round(y), col:pal[(Math.random()*pal.length)|0] });
  needsRender = true;
  api.dirty();
}
...
function render(){
  const gw = Math.ceil(api.W/RES), gh = Math.ceil(api.H/RES);
  if (!small){ small = document.createElement("canvas"); sm = small.getContext("2d"); }
  small.width = gw; small.height = gh;
  const img = sm.createImageData(gw, gh);
  const d = img.data;
  const own = new Int16Array(gw*gh);
  const cols = sites.map(s => hexToRgb(s.col));
  for (let j = 0; j < gh; j++){
    const y = j*RES;
    for (let i = 0; i < gw; i++){
      const x = i*RES;
      ...
      for (let k = 0; k < sites.length; k++){
        const dx = sites[k].x-x, dy = sites[k].y-y;
      ...
      const p = (j*gw+i)*4;
      if (bi >= 0){
        d[p] = cols[bi][0]; d[p+1] = cols[bi][1]; d[p+2] = cols[bi][2]; d[p+3] = 255;
      }
    }
  }
  // lead lines between differing cells
  const lead = Math.round(api.P.lead);
  const dark = api.bg() === "dark" ? [8,8,14] : [30,26,24];
  for (let j = 0; j < gh; j++)
    for (let i = 0; i < gw; i++){
      const k = j*gw+i, o = own[k];
      let edge = false;
      if (i+1 < gw && own[k+1] !== o) edge = true;
      else if (j+1 < gh && own[k+gw] !== o) edge = true;
      if (!edge) continue;
      for (let dj = -lead; dj <= lead; dj++)
        for (let di = -lead; di <= lead; di++){
          const jj = j+dj, ii = i+di;
          if (ii < 0 || jj < 0 || ii >= gw || jj >= gh) continue;
          const p = (jj*gw+ii)*4;
          d[p] = dark[0]; d[p+1] = dark[1]; d[p+2] = dark[2];
        }
    }
  ...
  if (sites.length){
    ...
    api.ctx.drawImage(small, 0, 0, api.W, api.H);
  }
}
window.TOOL = {
  id:"voronoi", file:"stainedglass.art",
  params:[
    { k:"pal", t:"select", l:"Glass", v:"jewel",
      opts:[["jewel","Jewel"],["warm","Warm"],["cool","Cool"],["forest","Forest"]] },
    { k:"lead", t:"range", l:"Lead", min:0, max:4, step:1, v:1, u:"px" },
    { k:"jitter", t:"button", l:"⤮ Jitter", fn:() => {
        for (const s of sites){ s.x += (Math.random()-.5)*30; s.y += (Math.random()-.5)*30; }
        needsRender = true; api.dirty();
      } },
    { k:"recolor", t:"button", l:"🎨 Recolor", fn:() => {
        const pal = PALETTES[api.P.pal];
        for (const s of sites) s.col = pal[(Math.random()*pal.length)|0];
        needsRender = true; api.dirty();
      } },
  ],
  init(a){ api = a; },
  onParam(){ needsRender = true; },
  bgChanged(){ needsRender = true; },
  pointer(type, x, y){
    if (type === "down"){
      dragIdx = nearSite(x, y);
      if (dragIdx < 0) addSite(x, y);
    } else if (type === "move" && dragIdx >= 0){
      sites[dragIdx].x = Math.round(x); sites[dragIdx].y = Math.round(y);
      needsRender = true; api.dirty();
    } else if (type === "up"){
      dragIdx = -1;
    }
  ...
  frame(){
    ...
    if (sites.length > 240) sites.length = 240; // keep the render fast
    render();
    needsRender = false;
  },
  clear(){ sites = []; needsRender = true; },
  serialize(){ return { sites }; },
  restore(s){ sites = (s && s.sites) || []; needsRender = true; },
};
})();
