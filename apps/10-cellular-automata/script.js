...
/* #20 Cellular Automata Art - paint live cells, press Run, and Conway-style
 * rules evolve the grid; cells shift hue as they age. */
(function(){
const CELL = 5;
const RULES = {
  life:     { b:[3],       s:[2,3] },
...
let api, gw, gh, grid, next, acc = 0, small, sm;
function reset(){
  gw = Math.floor(api.W/CELL); gh = Math.floor(api.H/CELL);
  grid = new Uint16Array(gw*gh); next = new Uint16Array(gw*gh);
  small = document.createElement("canvas");
  small.width = gw; small.height = gh;
  sm = small.getContext("2d");
}
function soup(){
  for (let k = 0; k < grid.length; k++) grid[k] = Math.random() < .16 ? 1 : 0;
  api.dirty();
}
function step(){
  const rule = RULES[api.P.rule] || RULES.life;
  for (let j = 0; j < gh; j++){
    const jm = ((j-1+gh)%gh)*gw, j0 = j*gw, jp = ((j+1)%gh)*gw;
    for (let i = 0; i < gw; i++){
      const im = (i-1+gw)%gw, ip = (i+1)%gw;
      let n = 0;
      if (grid[jm+im]) n++; if (grid[jm+i]) n++; if (grid[jm+ip]) n++;
      if (grid[j0+im]) n++;                      if (grid[j0+ip]) n++;
      if (grid[jp+im]) n++; if (grid[jp+i]) n++; if (grid[jp+ip]) n++;
      const alive = grid[j0+i] > 0;
      if (alive) next[j0+i] = rule.s.indexOf(n) >= 0 ? Math.min(600, grid[j0+i]+1) : 0;
      else       next[j0+i] = rule.b.indexOf(n) >= 0 ? 1 : 0;
    }
  }
...
function render(){
  const img = sm.createImageData(gw, gh), d = img.data;
  const hueBase = api.P.hue;
  for (let k = 0; k < grid.length; k++){
    const age = grid[k];
    if (!age) continue;
    const [r, g, b] = hslToRgb(hueBase + Math.min(age, 90)*2.4, 80, api.bg() === "dark" ? 56 : 42);
    const p = k*4;
  ...
  api.ctx.drawImage(small, 0, 0, gw*CELL, gh*CELL);
}
window.TOY = {
  id:"cellart", file:"cellart.art", pixelated:true,
  params:[
    { k:"rule", t:"select", l:"Rule", v:"life",
      opts:[["life","Life"],["highlife","HighLife"],["seeds","Seeds"],["daynight","Day&Night"],["maze","Maze"]] },
    { k:"run", t:"toggle", l:"Run", v:false },
    { k:"speed", t:"range", l:"Speed", min:1, max:30, step:1, v:10, u:"gen/s" },
    { k:"brush", t:"range", l:"Brush", min:1, max:6, step:1, v:2, u:"c" },
    { k:"hue", t:"range", l:"Hue", min:0, max:360, step:5, v:120, u:"°" },
    { k:"soup", t:"button", l:"🎲 Soup", fn:soup },
    { k:"stepBtn", t:"button", l:"⏭ Step", fn:step },
  ],
  init(a){ api = a; reset(); },
  pointer(type, x, y){
    ...
    if (!this._dn) return;
    const ci = Math.floor(x/CELL), cj = Math.floor(y/CELL), r = api.P.brush;
    for (let dj = -r+1; dj < r; dj++)
      for (let di = -r+1; di < r; di++){
        const ii = ci+di, jj = cj+dj;
        if (ii < 0 || jj < 0 || ii >= gw || jj >= gh) continue;
        if (di*di + dj*dj < r*r) grid[jj*gw+ii] = 1;
      }
    api.dirty();
  },
  frame(dt){
  ...
  serialize(){
    const cells = [];
    for (let k = 0; k < grid.length; k++) if (grid[k]) cells.push([k, grid[k]]);
    return { gw, gh, cells };
  },
  restore(s){
    grid.fill(0);
    if (!s || !s.cells) return;
    for (const [k, age] of s.cells){
      if (s.gw === gw){ if (k < grid.length) grid[k] = age; }
      else {
        const sj = Math.floor(k/s.gw), si = k%s.gw;
        if (si < gw && sj < gh) grid[sj*gw+si] = age;
      }
    }
  },
};
