...
/* #02 Digital Spirograph - gear ratio + pen offset trace looping curves.
 * Live preview animates at the drag position; Stamp bakes it to the canvas. */
(function(){
let api, t = 0, pos = null, curves = [];
function gcd(a, b){ while (b){ const t = b; b = a % b; a = t; } return a || 1; }
function cur(){
  return { R:api.P.R, r:api.P.r, d:api.P.d, w:api.P.w, col:api.P.col, x:pos[0], y:pos[1] };
}
function period(c){ return Math.PI*2*(c.r/gcd(Math.round(c.R), Math.round(c.r))); }
function drawCurve(ctx, c, thMax){
  const n = Math.min(9000, Math.max(80, Math.floor(thMax*22)));
  const q = (c.R - c.r)/c.r;
  ctx.strokeStyle = c.col; ctx.lineWidth = c.w; ctx.lineJoin = "round";
  ctx.beginPath();
  for (let i = 0; i <= n; i++){
    const th = thMax*i/n;
    const x = c.x + (c.R-c.r)*Math.cos(th) + c.d*Math.cos(q*th);
    const y = c.y + (c.R-c.r)*Math.sin(th) - c.d*Math.sin(q*th);
...
window.TOY = {
  ...
  params:[
    ...
    { k:"r", t:"range", l:"Gear",  min:8,  max:160, step:1, v:62,  u:"px" },
    { k:"d", t:"range", l:"Pen",   min:0,  max:200, step:1, v:88,  u:"px" },
    { k:"speed", t:"range", l:"Speed", min:1, max:24, step:1, v:9 },
    { k:"w", t:"range", l:"Line",  min:.5, max:6, step:.5, v:1.5, u:"px" },
    { k:"col", t:"color", l:"Color", v:"#ff9de2" },
    { k:"stamp", t:"button", l:"✚ Stamp", fn:stamp },
  ],
  init(a){ api = a; pos = [a.W/2, a.H/2]; },
  pointer(type, x, y){ if (type === "down" || type === "move") { /* drag repositions */ }
    if (type === "down"){ this._drag = true; pos = [x, y]; }
    else if (type === "move" && this._drag){ pos = [x, y]; }
    else if (type === "up"){ this._drag = false; }
  },
  frame(dt){ t += dt*api.P.speed; const T = period(cur()); if (t > T + 1.2) t = 0; },
  overlay(c){
    drawCurve(c, cur(), Math.min(t, period(cur())));
    c.fillStyle = "rgba(128,128,160,.5)";
    c.beginPath(); c.arc(pos[0], pos[1], 3, 0, 7); c.fill();
  },
  onParam(){ t = 0; },
  clear(){ curves = []; api.clearWorld(); },
  serialize(){ return { curves }; },
  restore(s){
    curves = (s && s.curves) || [];
    api.clearWorld();
    for (const c of curves) drawCurve(api.ctx, c, period(c));
  },
};
})();
